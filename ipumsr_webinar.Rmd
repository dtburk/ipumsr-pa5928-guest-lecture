...
7. Working with IPUMS geographic data
...
![Trends in multigenerational households by income level and race/ethnicity](images/example_usa1.png)
...
![Table showing correlation between hearing impairment and mortality](images/example_health1.png)
...
![visualization showing individual dots moving between activities](images/example_time_use.png)
...
![Maps showing individual educational attainment by region](images/example_nhgis2.png)
...
knitr::include_graphics("images/transparent_blank.png")
```
...
The API functions are in the development version. The API itself is not yet 
publicly available, but email IPUMS support at ipums@umn.edu if you're 
...
```{r echo=FALSE, fig.alt="Columns of green symbols streaming down a black background"}
knitr::include_graphics("images/matrix-code.gif")
```

.imgsrc[
  Source: https://c.tenor.com/KBe_nw4IL2QAAAAC/matrix-code.gif
]
...
# The DDI codebook: Decoding The Matrix
...
```{r echo=FALSE, fig.alt="Columns of green symbols streaming on a black background, forming the shape of three men standing at the other end of a hallway"}
knitr::include_graphics("images/the-matrix-matrix.gif")
```

.imgsrc[
  Source: https://c.tenor.com/uJ_GkLiWhcYAAAAC/the-matrix-matrix.gif
]
...
knitr::include_graphics("images/comparability_screenshot.png")
```
...
.strong[`7.` Working with IPUMS geographic data]
...
knitr::include_graphics("images/geography_screenshot.png")
```
...
- Interested in becoming a beta tester? Email ipums@umn.edu
...
```{r eval=FALSE}
old_extract <- get_last_extract_info("usa")
```

--

Or if we know the number of the extract we want:
...
```{r eval=FALSE}
old_extract %>% 
  revise_extract_micro(vars_to_add = "EDUC") %>% 
  submit_extract()
```
...
- Create an issue on GitHub: https://github.com/ipums/ipumsr

- This presentation: https://github.com/dtburk/ipumsr-pa5928-guest-lecture
